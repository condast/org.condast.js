...
public class Plotter extends Canvas {
	 ...
	 */
	public static final int BASE_OFFSET= 75;	
	private static final int MAX_BASE_OFFSET =100;	
	private static final int MIN_BASE_OFFSET = -100;

	/**
	 * The default cursor starting position.
	 */
	private static final int CURSOR_START_DEFAULT = 5;
	 ...
	 */
	private static final int STEADYPOSITION_75PERCENT = -1;
		
	private static final int MIN_TAIL_FADE = 1;
	
	private static final int MAX_TAIL_FADE = 100;

	/**
	 * The default tail fade percentage
	 */
	private static final int TAILFADE_PERCENTAGE = 25;

	/**
	 * The default tail size is 75% of the width.
	 */
	public static final int TAILSIZE_DEFAULT = 25;
	...
	private static final boolean IS_PERCENTAGE = false;
	
	private static final boolean IS_ANTI_ALIAS = false;
	/**
	 ...
	 */
	private static final int TAILSIZE_FILL = -2;
	...
	private static final int BACKGROUND_COLOR_CURSOR = SWT.COLOR_WHITE;	
	private static final int FOREGROUND_COLOR_CURSOR = SWT.COLOR_WHITE;
		
	private static final int DEFAULT_RANGE_HIGH_VALUE = 100;	
	private static final int DEFAULT_RANGE_LOW_VALUE = -100;
	
	private int myRangeHighValue=DEFAULT_RANGE_HIGH_VALUE;
	private int myRangeLowValue=DEFAULT_RANGE_LOW_VALUE;	
	...
	public Plotter(int channels, Composite parent, int style, Color backgroundColor, Color foregroundColor) {
		...
		if (backgroundColor == null) {
			this.bg = Display.getDefault().getSystemColor(BACKGROUND_COLOR_CURSOR);
		} else {
			this.bg = backgroundColor;
		}
		Color fg;
		if (foregroundColor == null) {
			fg = Display.getDefault().getSystemColor(FOREGROUND_COLOR_CURSOR);
		} else {
			fg = foregroundColor;
		}
		...
		this.chan = new Channel[channels];
		for (int i = 0; i < this.chan.length; i++) {

			this.chan[i] = new Channel();
			this.chan[i].fg = fg;
	...
	public void setRangeLowValue(int lowValue){
	...
	public void setRangeHighValue(int highValue){
	...
	public void setBaseOffset(int channel, int newBaseOffset) {
		...
		if (baseOffset > MAX_BASE_OFFSET) {
			baseOffset = MAX_BASE_OFFSET;
		}

		if (baseOffset < MIN_BASE_OFFSET) {
			baseOffset = MIN_BASE_OFFSET;
		}
	...
	public void setTailFade(int channel, int newTailFade) {
		...
		checkWidget();
		if (tailFade > MAX_TAIL_FADE) {
			tailFade = 100;
		}
		if (tailFade < MIN_TAIL_FADE) {
			tailFade = 1;
		}
	...
	private class Channel {
		...
		private int base;
		private int baseOffset = BASE_OFFSET;
		private boolean connect;
		private int cursor = CURSOR_START_DEFAULT;
		private boolean fade;
		private Color fg=Display.getDefault().getSystemColor(FOREGROUND_COLOR_CURSOR);
		...
		private boolean percentage = IS_PERCENTAGE;
		...
		private boolean antiAlias = IS_ANTI_ALIAS;
